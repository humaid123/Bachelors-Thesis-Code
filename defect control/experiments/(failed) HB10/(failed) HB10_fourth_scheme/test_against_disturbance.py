    ...
    for disturbance in [1e-6, 1e-7, 1e-8, 1e-9]:
        ...
        for h in the_hs:
            ...
            for x0 in [0, 0.1, 1, 2, 5, 4.633, 10]:
                ...
                (x_i_minus_0_5, y_i_minus_0_5)   =    x_i_minus_1 + h, solution([ x_i_minus_1 + h ])[0] + disturbance
                ...
                # beta up to 3 => for trial in [0.1, 0.3, 0.7, 1.2, 1.5, 1.7, 2, 2.3, 2.5, 2.7, 3]:
                # beta up to 1 => 
        ...
        # plt.title(f"error at a disturbance of {disturbance}")
        plt.show()

        plt.figure()
        plt.plot(log_hs, log_max_defect_at_each_h)
        # plt.title(f"defect at a disturbance of {disturbance}")
        plt.show()

        plt.figure()
        plt.plot(log_hs, log_max_error_horner_at_each_h)
        # plt.title(f"error horner at a disturbance of {disturbance}")
        plt.show()

        plt.figure()
        plt.plot(log_hs, log_max_defect_horner_at_each_h)
        # plt.title(f"defect horner at a disturbance of {disturbance}")
        plt.show()
        ...
        print("order of method is", maximum, "at a disturbance of", disturbance)
        maximum = 0
        for i in range(len(log_max_defect_at_each_h) - 1):
            value = log_max_defect_at_each_h[i] - log_max_defect_at_each_h[i + 1]
            # print("value", value)
            maximum = max(maximum, value)
        print("order of prime", maximum, "at a disturbance of", disturbance)
        ...
        for i in range(len(log_max_error_horner_at_each_h) - 1):
            ...
            # print("value", value)
            maximum = max(maximum, value)
        print("order of method in horner form", maximum, "at a disturbance of", disturbance)
        maximum = 0
        for i in range(len(log_max_defect_horner_at_each_h) - 1):
            ...
            # print("value", value)
            maximum = max(maximum, value)
        print("order of prime in horner form", maximum, "at a disturbance of", disturbance)
