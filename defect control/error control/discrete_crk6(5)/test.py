...
def experiment(model, y0, t_span, solution):
    ...
    tol = 1e-6
    (res, continous_sol, lower_continous_sol) = rk_error_control(model, t_span, y0[0], tol)
    ...
    lower_computed_solutions = [lower_continous_sol.eval(x) for x in t_eval]
    plt.plot(t_eval, computed_solutions, label="higher order")
    plt.plot(t_eval, lower_computed_solutions, label="lower order")
    ...
    lower_order_error = [abs(computed_solution - actual_solution) for (computed_solution, actual_solution) in zip(lower_computed_solutions, actual_solutions)]
    for this_x in xs:
        plt.axvline(x=this_x) 
    plt.plot(t_eval, error, label="higher order")
    plt.plot(t_eval, lower_order_error, label="lower order")
    ...
    plot_shapes_of_error(continous_sol, solution, "higher order")
    plot_shapes_of_error(lower_continous_sol, solution, "lower order")
    plot_shapes_of_error_two_orders(continous_sol, lower_continous_sol)
