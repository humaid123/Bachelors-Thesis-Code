c BACOLI-4 (BACOL with interpolation-based spatial error estimation,
c           LAMPAK in place of COLROW, and DASKR in place of DASSL)
...
c Copyright (c) 2019, Paul Muir, Jack Pew, Connor Tannahill
c Paul Muir,        Mathematics and Computing Science,
c                   Saint Mary's University.
c Connor Tannahill, Mathematics and Computing Science,
c                   Saint Mary's University.
...
c 1. The modified supporting routines collected in bacoli-aux.f.
c 2. A driving program, e.g., driver-curve.f.
c 3. PDE system definition subroutines, e.g., RCDsys.f.
...
c       BACOLI-3 -> BACOLI-4
c
c       When using the event detection capabilities of BACOLI-4, set
c       nrt > 0 (the number of events) and provide the subroutine
c
c       rt(t, x, nint, ub, utb, neq, rval, nrt)
c       t is a double precision scalar, x is a double precision vector
c       of length nint+1, ub is a double precision vector of length
c       neq, utb is a double precision vector of length neq,
c       rval is a double precision vector of length nrt.
c
c-----------------------------------------------------------------------
        implicit none
c-----------------------------------------------------------------------
...
c                               of BACOLI with DASKR and which error
...
c                               indicates that BACOLI and DASKR should
...
c
c                               After a return from BACOLI, to force
c                               a cold start on the next step, set
c                               mflag(1) = 2. This is mostly used
c                               in the setting of event detection, where
c                               the PDE may change after some event,
c                               requiring the time integration to be
c                               completely restarted from the location
c                               of the event.
c
...
c                               Since DASKR may actually "step over"
...
c                               If the user wants to use DASKR with the
...
c
        external                rt
c                               In the event detection mode, defines 
c                               the functions
c                                Ri(t, x, u(x, t), ux(x,t)), i=1,..,nrt.
c                               The roots of this function are then
c                               searched for during the computation.
c                               
c                               Subroutine defining events which will
c                               be searched for during the computation.

c
        external                rtbac
c                               Internal rt routine
c
        integer                 nrt
c                               Number of events being searched for.
c
        integer                 jroot(nrt)
c                               Outputs root information
c
c                               if jroot(i) = -1 or 1, i = 1,..,nrt
c                               then Ri(t, x, u(x,t), ux(x,t)) has a
c                               root at t
c
c                               jroot(i) = 0 otherwise
c
c       Work Storage:
...
c                               DASKR plus some additional status codes
...
c
c                               An excerpt from the DASKR source code
c                               documentation is included to define
c                               the idid return codes and clarify
c                               the operation of DASKR within BACOLI.
c
c-----------------------------------------------------------------------
c  The following is an excerpt from the DASKR source code documentation:
c         (Note: any reference to DASKR's capability for using
c                iterative methods can be ignored, as these are not
c                implemented in BACOLI-4)
c
...
c----------------------------------------------------------------------
C
C   If DASKR is called again without any action taken to remove the
...
C
c-----------------------------------------------------------------------
c Notes regarding modifications:
c
...
c
c The structure of and several pointers into both rpar and ipar was
...
c DASKR was given a smaller number of equations for time stepping.
...
c the value of tstop in rwork(itstop) became a garbage on a cold start
c which could be caught as invalid input for DASKR.
c
c   BACOLI -> BACOLI-2
c
c BACOLI now takes the user subroutines, which define the PDE system,
c as external parameters. These parameters had to be added to several
c BACOLI and DASKR subroutines to have them passed down to CALRES,
...
c   BACOLI-2 -> BACOLI-3
c
c BACOLI will now use finite differences to approximate iteration
...
c bndxa and bndxb. DDASKR was modified further to take a new MTYPE = 6
c which signals that approximated ABD linear algebra should be done
c (MTYPE = 3 is for dense ABD lin.alg.), and DDAJAC was modified to
c implement this through a minimal number of calls to RES.
...
c                               methods used by DASKR. maxord = 5 is
c                               the default used by DASKR.
c
        integer                 kerr
...
c                               integer work array required by daskr.
c
        integer                 lenpd
...
c                               point work array used by RES and JACD.
...
c
        integer                 lenbwk
c                               lenbwrk is the size of the floating
c                               point work array used by the B-spline
c                               basis function evaluation routine,
c                               DBSPVD, which is called from various
c                               other subroutines.
c
        integer                 ncpts
...
c                               remeshing when using daskr_{kcol}.
c
        integer                 irold
...
c
        integer                 ididtemp
c                               stores idid after root discovery
c-----------------------------------------------------------------------
c Loop indices:
        integer                 i
c
...
c                               the BDF methods employed in DASKR.
...
c                               point work array for DASKR, and is
...
c
c        integer                 iwkdnm
c        parameter              (iwkdnm = 51)
cc                               rpar(ipar(iwkdnm)) is the work storage
cc                               for the modification version of the
cc                               subroutine DDANRM.
c
        integer                 ixold
        parameter              (ixold  = 52)
...
c                               remeshing, the first 60 elements of the
c                               floating point work array in daskr.
c
        integer                 iypre
        parameter              (iypre  = 54)
c                               rpar(ipar(iypre)) stores the values of
c                               rpar(ipar(iy)) at the previous 6 steps.
c                               It is required for a warm restart after
c                               remeshing.
c
        integer                 iyprer
        parameter              (iyprer = 55)
...
c
        integer                 ibwrk
        parameter              (ibwrk  = 57)
c                               rpar(ipar(ibwrk)) is work space for
c                               B-spline basis function evaluation.
c-----------------------------------------------------------------------
c Subroutines Passed to DASKR:
        external                jacd
        external                res
        external                psol
c
...
c     Check validity of the mflag vector.
   ...
   1  continue
      ...
      if ((mflag(1) .lt. 0) .or. (mflag(1) .gt. 2)) goto 712
...
c     Is the maximum order BDF method used restricted by the user?
      maxord = 5
      if (mflag(7) .eq. 1) maxord = ipar(imxord)

c     Check for continuation of a previous problem.
      if ((mflag(1) .eq. 1) .or. (mflag(1) .eq. 2)) then
         ...
         leniw = 40 + neq
         lenrw = 60 + max(maxord+4,7) * neq + lenpd + neq + 3*nrt
...
C     If a cold start is requested by the user (mflag(1) = 2) then
C     reset the flags

          if (mflag(1) .eq. 2) then
C            Log this cold start
             ipar(icolds) = ipar(icolds) + 1

             mflag(1) = 1

c            Set the info vector required by DASKR.
c            Is this a continuation of a previous problem?
c            Should not be seen as a continuation
             ipar(iinfo-1+1) = 0
c            Are vectors used for tolerances?
             ipar(iinfo-1+2) = mflag(2)
c            BACOLI employs DASKR's intermediate-output mode.
             ipar(iinfo-1+3) = 1
c            May DASKR step past tout?
             ipar(iinfo-1+4) = mflag(3)
c            Indicate to DASKR whether or not an analytic Jacobian
c            (iteration) matrix is supplied. For the purposes of DASKR,
...
c            What is the Jacobian structure? (dense=0, banded=1, ABD=2)
             ipar(iinfo-1+6) = 2
c            BACOLI does not currently support a maximum stepsize (TODO)
             ipar(iinfo-1+7) = 0
c            Indicate whether an user-supplied initial stepsize is used.
             ipar(iinfo-1+8) = mflag(6)
c            Is the maximum order of the BDF method restricted < maxord?
             ipar(iinfo-1+9) = mflag(7)
c            BACOLI does not support constraining DAE components.
c            The DAE components are DAE coefficients, so it would be tricky.
             ipar(iinfo-1+10) = 0
c            BACOLI computes its own consistent initial conditions.
             ipar(iinfo-1+11) = 0
             ipar(iinfo-1+14) = 0
             ipar(iinfo-1+17) = 0
             ipar(iinfo-1+18) = 0
c            BACOLI does not currently support Krylov methods (TODO)
             ipar(iinfo-1+12) = 0
             ipar(iinfo-1+13) = 0
             ipar(iinfo-1+15) = 0
c            Algebraic variables in the DAE system are not omitted from
c            error tests.
             ipar(iinfo-1+16) = 0
          end if
...
c-----------------------------------------------------------------------
c     Calculate the extra storage requirements of res and jacd.
      lenrj = (6 + 5 * npde) * npde

c     Calculate the number of collocation points.
      ncpts = nint * kcol + nconti

c     Calculate the number of DAEs given to daskr.
      neq = npde * ncpts

c     Size of the ABD iteration matrix in daskr.
      lenpd = npde*npde*(nconti+nconti+kcol*(kcol+nconti)*nint)

c     Calculate the extra storage requirements of iniy and iniyp.
      lenin = lenpd + 3 * neq + 2 * npde * (2 + npde)

c     Calculate the extra storage requirements of reinit.
      lenri = lenpd + kcol + nconti + kcol * (ninpre + 1)
     &         + 2 * nconti + neq

c     Calculate the extra storage requirements of dbspvd.
      lenbwk = (kcol+nconti+1)*(kcol+nconti+2)/2

c-----------------------------------------------------------------------
c     Total size of the DASKR floating point work array.
      lenrw = 60 + max(maxord+4,7) * neq + lenpd + neq

      lenrw = lenrw + 3*nrt

c     Total size of the DASKR integer work array.
      leniw = 40 + neq
...
c      ipar(iwkdnm) = ipar(iework) + lenerr
      ipar(ibwrk)  = ipar(iework) + lenerr

c      ipar(iyprer) = ipar(iwkdnm) + neq
      ipar(iyprer) = ipar(ibwrk)  + lenbwk
      ipar(ixold)  = ipar(iyprer) + (maxord+1) * neq
      ipar(idasr)  = ipar(ixold)  + nintmx + 1
      ipar(iypre)  = ipar(idasr)  + 60

      ipar(iey)    = ipar(irwork) + 60 + 3 * neq

c     This offset is different between the initial call and remeshing.
      if (((irshfg .ne. 0) .and. (istart .eq. 1))) then
         ipar(irpstp) = ipar(iypre) + (maxord+1) * neqpre - 1
      else
         ipar(irpstp) = ipar(iypre) + (maxord+1) * neq - 1
      endif

c     Check for a sufficiently large rpar floating point work array.
      if (lrp .lt. ipar(irpstp)) then
        goto 770
      end if
...
c     A different set of initializations is done in each case.
      if ((irshfg .ne. 0) .and. (istart .ne. 0)) then
          goto 300
      end if
...
c        This is a remeshing at the initial step. Increment the counter.
     ...
     &          mflag(9), rpar(ipar(ibwrk)), lenbwk,
     &          uinit, bndxa, difbxa, bndxb, difbxb)
...
c     This is inside the integer work array of daskr. Those parameter
c     include npde, kcol, nint.
      ipar(iiwork-1+38) = npde
      ipar(iiwork-1+39) = kcol
      ipar(iiwork-1+40) = nint
...
c     Set the maximum BDF order for DASKR if applicable.
...
c     Examine idid to determine if DASKR can be called again.
      if (idid .ne. 1) goto 790

c     If t0 must not go beyond tstop in the time stepping, then update
c     the DASKR floating point work array with the value of tstop.
      if (mflag(3) .eq. 1) then
         rpar(ipar(irwork)) = rpar(itstop)
...
c     Reset t0. If necessary, update rpar(ipar(iy)), rpar(ipar(iyp)).
     ...
     &               rpar(ipar(irwork)+60+3*neq),
     &               rpar(ipar(irwork)-1+39))
         idid = 3
         goto 500
      endif
...
c     Tell DASKR to calculate the new iteration matrix and reset the
c     size of the iteration matrix.
      ipar(iiwork-1+5) = -1
      ipar(iiwork-1+22) = lenpd

c     Reset the ABD information in DASKR.
      ipar(iiwork-1+40) = nint
...
c        rpar is shrinking; copy from the front
         do i = 1, nintmx+1+60+6*neqpre
            rpar(ipar(ixold)-1+i) = rpar(irold-1+i)
         end do
      else if (nint .gt. ninold) then
c        rpar is growing; copy in reverse
         do i = nintmx+1+60+6*neqpre, 1, -1
            rpar(ipar(ixold)-1+i) = rpar(irold-1+i)
         end do
      endif

c     Reset the first 60 elements of the floating point work array in
c     DASKR.
      call dcopy(60, rpar(ipar(idasr)), 1, rpar(ipar(irwork)), 1)

      call meshsq(kerr, nint, x, rpar(ipar(irwork)+60), rpar(ipar(ih)),
     &            rpar(ipar(iexcol)), rpar(ipar(iewts)))
...
c     reinit uses rpar(ipar(irwork)+60+6*neq) as its work array and
c     outputs y to rpar(ipar(irwork)+60).
      call reinit(npde, kcol, kcol, nint, ninpre, ncpts, neq,
     &            neqpre, icount, ipar(idasi-1+11), nstep, x,
     &            rpar(ipar(ixold)), rpar(ipar(iypre)), 0,
     &            rpar(ipar(irwork)+60+6*neq), lenri,
     &            rpar(ipar(ibwrk)), lenbwk, ipar(ipivot),
     &            rpar(ipar(ih)), rpar(ipar(ixbs)), rpar(ipar(ixcol)),
     &            rpar(ipar(ibasi)), rpar(ipar(irwork)+60),
...
c     cold start.
      if (irshfg .eq. 2) then
         call dcopy(neq, rpar(ipar(irwork)+60), 1, rpar(ipar(iy)), 1)
         call iniyp(t0, npde, kcol, nint, neq, ncpts,
     &              rpar(ipar(ixcol)), rpar(ipar(iabtop)),
     &              rpar(ipar(iabblk)), rpar(ipar(iabbot)),
...
c     generated by reinit for a warm start.
      call dcopy(ipar(idasi-1+8)*neq, rpar(ipar(irwork)+60),
     &           1, rpar(ipar(iyprer)), 1)

c     Do divided differences to convert the interpolated B-spline
c     coefficients to the form used by DASKR - several derivatives
...
c     array that is normally used by ERREST.)
      call divdif(neq, ipar(idasi-1+8)+1, rpar(ipar(idasr)-1+39),
     &            rpar(ipar(iework)), rpar(ipar(irwork)+60))

  320 continue

c     Move the B-spline coefficients from where reinit put them
c     to where DASKR expects to find them.
...
c     position is ahead of the old one.
      do i = neq*(ipar(iiwork-1+8)+1), 1, -1
         rpar(ipar(irwork)+60+3*neq-1+i) = rpar(ipar(irwork)+60-1+i)
      end do

c-----------------------------------------------------------------------
c     Time integration loop for DASKR.
c-----------------------------------------------------------------------

  400 continue

c     Backup idid status. idid can't be set to 5 if false
c     root found (e.g. a root found but solution doesn't meet
c     the tolerance)
      ididtemp = idid

      call ddaskr(res, neq, t0, rpar(ipar(iy)), rpar(ipar(iyp)),
     &            tout, ipar(iinfo), rpar(ipar(itrtol)),
     &            rpar(ipar(itatol)), idid, rpar(ipar(irwork)), lenrw,
     &            ipar(iiwork), leniw, rpar, ipar, jacd, psol,
     &            rtbac, nrt, jroot, f, derivf, bndxa, difbxa, bndxb,
     &            difbxb, x, rt)
...
c-----------------------------------------------------------------------
     ...
     &            lenbwk, rpar(ipar(ibwrk)), rpar(ipar(iebasi)),
     &            rpar(ipar(iecoef)), lencof, errrat,
     &            rpar(ipar(ierint)), rpar(ipar(iercom)),
     &            ieflag, x, rpar(ipar(ih)), mflag(8))

      if (ieflag .eq. 0) then

c        The current step is accepted.
         write(7,*) 'This step is also accepted by BACOLIKR'
...
c           Check for a root on the current step, if so return control
c           to user.
            if (idid .eq. 5) then
               goto 500
            endif
...
c           In the first step after a remeshing, we do not allow DASKR
...
c-----------------------------------------------------------------------
  600 continue
      ...
      write(6,9999) '       been issued by DASKR.'
      return
  610 continue
      if (istart .eq. 1) then
...
c-----------------------------------------------------------------------
  ...
  712 continue
      write(6,9999) 'ERROR: BACOLI input violation.'
      write(6,9999) 'Require:  0 <= mflag(1) <= 2'
      idid = -51
      return
  715 continue
      write(6,9999) 'ERROR: BACOLI input violation.'
...
c
        integer                 npts
c       npts is the number of points in the xsol vector.
c
...
c       Work Storage:
     ...
     *                               +(kcol+nconti+1)*(kcol+nconti+2)/2
     *                               +2*kcol*(nint+1)+2*nconti)
...
c
        integer                 mflag
c                               mflag is required by subroutine dintrv.
c
        integer                 ilo
c                               ilo is required by subroutine dintrv.
c
...
c
        integer                 ibwrk
c                               work(ibwrk) is work storage for dbspvd.
c
c-----------------------------------------------------------------------
c Loop indices:
        integer                 i
...
c     set the pointer into the floating point work array
      ibwrk = 1 + (kcol+nconti)*(nderiv+1)
      ixbs  = ibwrk + (kcol+nconti+1)*(kcol+nconti+2)/2
...
c
   ...
   30 continue
      ...
      do 70 i = 1, npts
c
c     dintrv is called to compute ileft. dbspvd is called to compute
c     the values of the basis function at the required point.
         call dintrv(work(ixbs), ncpts, xsol(i), ilo, ileft, mflag)
         call dbspvd(work(ixbs), kcol+nconti, nderiv+1, xsol(i), ileft,
     &               kcol+nconti, work, work(ibwrk))
   ...
   70 continue
     ...
     &                  xbs, xsol, y, errbas, usol, lwork, lbw)
...
c-----------------------------------------------------------------------
c
c Modified by Rong Wang, August 29, 2001.
c Last modified by Jack Pew, August 1, 2013.
c
c-----------------------------------------------------------------------
        implicit none
c Constants:
...
c
        integer                 lbw
c                               lenbwk is the size of the work array
c                               for dbspvd and must satisfy:
c                               lbw >= (kcol+nconti+1)*(kcol+nconti+2)/2
c
c       output:
...
c
c       Work storage:
        double precision        lwork(lbw)
c                               Work storage for dbspvd.
c
c-----------------------------------------------------------------------
c Local Variables:
        integer                 ileft
...
c     calculate errbas.
      do i = 1, nint
         ...
         do j = 1, nptse
            jj = (i - 1) * nptse + j
            call dbspvd(xbs, kcol+nconti, 1, xsol(jj), ileft,
     &                  kcol+nconti, errbas(1,jj), lwork)
         end do
      end do
...
c     compute the values of usol at xsol.
   ...
   70 continue
     ...
     &                  rtol, lenwk, work, lbw, lwork, errbas, ercoef,
     &                  lencof,
...
c
        double precision        xbs(kcol*(nint+1)+nconti+nconti)
c                               xbs is the breakpoint sequence when
c                               using daskr_kcol.
c
        double precision        y(neq)
c                               y is the vector of bspline
c                               coefficients when using daskr_kcol.
c
        integer                 istart
c                               istart is a flag to begin the code.
c                               istart = 0, it is the initial step;
...
c     evaluate the interpolant.
     ...
     &            xbs, xsol, y, errbas, work(iusol1), lwork, lbw)
...
c       accepted time step (i.e. no need remeshing). This information
...
c                               DASKR has taken when using daskr.
c
        integer                 nstep
...
c
        integer                 icdas(40)
c                               icdas stores the first 40 elements of
c                               the integer work array in daskr.
c
        double precision        cdasr(60)
c                               cdasr stores the first 60 elements of
c                               the floating point work array in daskr.
c
...
c     Define the collocation point sequence.
   ...
   50 continue
     ...
     &                ifgfdj, lwork, lbw, uinit, bndxa, difbxa, bndxb,
     &                difbxb)
...
c
        integer                 irhs
c                               work(irhs-1+i), i=1, neq, is the rhs
c                               solved for when using lamsol.
c
c-----------------------------------------------------------------------
c Loop indices:
        integer                 i
...
c-----------------------------------------------------------------------
c     dbspvd is called to compute the components of fbasis(k,i,j)
c     associated the first collocation point. Now ileft = kcol + nconti.
      call dbspvd(xbs, kcol+nconti, 3, xcol(1), kcol+nconti,
     &            kcol+nconti, fbasis(1,1,1), lwork)
...
c     compute information for ii-th collocation point.
            call dbspvd(xbs, kcol+nconti, 3, xcol(ii), ileft,
     &                  kcol+nconti, fbasis(1,1,ii), lwork)
...
c     side.
c
      call dbspvd(xbs, kcol+nconti, 3, xcol(ncpts), ncpts,
     &            kcol+nconti, fbasis(1,1,ncpts), lwork)
      ii = neq - npde + 1
...
c     LU decompose the matrix.
      ...
      call lamdec(neq, work(iabdtp), npde, 2*npde, work(iabdbk),
     &            kcol*npde, (kcol+nconti)*npde, nint, work(iabdbt),
     &            npde, ipivot, icflag)
...
c     conditions, i.e. y(t0). If not, this gives the predictor of y(t0).
      call dcopy(neq, y, 1, work(irhs), 1)
...
c     Update the values at the right boundary.
      call eval(npde,kcol,ncpts,3,ncpts,work(iu),work(iux),
...
c     Solve the corrector equation.
      call dcopy(neq, work(idelta), 1, work(irhs), 1)
      call lamsol(work(iabdtp), npde, 2*npde, work(iabdbk), kcol*npde,
     &           (kcol+nconti)*npde, nint, work(iabdbt), npde, ipivot,
     &           work(irhs), work(idelta))
...
c           derivatives at the current collocation point.
            call eval(npde,kcol,ii,2,ncpts,work(iu),work(iux),
...
c     Solve the linear system. This gives yprime(t0)
      call dcopy(neq, yprime, 1, work(irhs), 1)
      call lamsol(work(iabdtp), npde, 2*npde, work(iabdbk), kcol*npde,
     &            (kcol+nconti)*npde, nint, work(iabdbt), npde, ipivot,
     &            work(irhs), yprime)
  ...
  999 continue
     ...
     &                  iflag, work, lw, lwork, lbw, ipivot, h, xbs,
     &                  xcol, fbasis, y, abdblk, icflag)
...
c
        integer                 istep
c                               istep is the number of time steps that
c                               DASKR has taken when kcol collocation
...
c     coefficients for the initial conditions, i.e. y(t0).
      do 90 i = 1, nstep
         ii = (i - 1) * neq + 1
         call dcopy(neq, y(ii), 1, work(irhs), 1)
         call lamsol(work(iabdtp), npde, 2*npde, work(iabdbk),
     &               kcol*npde, (kcol+nconti)*npde, nint, work(iabdbt),
     &               npde, ipivot, work(irhs), y(ii))
         if (icflag .ne. 0) go to 999
   90 continue
...
c-----------------------------------------------------------------------
c
c Modified by Rong Wang, Mar 31, 2006.
c Last modified by Jack Pew, August 1, 2013.
c
c-----------------------------------------------------------------------
        implicit none
...
c     the values of the basis function at the required point.
            call dintrv(work(ixbs), ncpts, xsol(i), ilo, ileft, mflag)
            call dbspvd(work(ixbs), kcol+nconti, 1, xsol(i), ileft,
     &                  kcol+nconti, work, lwork)
            ii = ileft - kcol - nconti
            do 50 k = 1, npde
               usol(k,i,n) = zero
               do 40 m = 1, kcol + nconti
...
c
        integer                 icpt
c                               if calling eval for the left or right
c                               collocation point, set icpt to 1 or 3.
c                               for central collocation points,
c                               set icpt to 2. This is done since
c                               several additional B-spline basis
c                               functions are known to be zero at the
c                               endpoints of the mesh.
c
        integer                 ncpts
c                               ncpts=(kcol*nint+nconti) is the number
c                               of collocation points.
...
c-----------------------------------------------------------------------
      ...
      if (icpt .eq. 2) then
         ...
         do j = 1, npde
            do m = 1, kcol + nconti
               uval(j)   = uval(j)   + fbasis(m,1) * y(j,m0+m)
               uxval(j)  = uxval(j)  + fbasis(m,2) * y(j,m0+m)
               uxxval(j) = uxxval(j) + fbasis(m,3) * y(j,m0+m)
            end do
         end do
      else if (icpt .eq. 1) then
         do j = 1, npde
            uval(j)   = fbasis(1,1) * y(j,1)
            uxval(j)  = fbasis(1,2) * y(j,1)
     &                + fbasis(2,2) * y(j,2)
            uxxval(j) = fbasis(1,3) * y(j,1)
     &                + fbasis(2,3) * y(j,2)
     &                + fbasis(3,3) * y(j,3)
         end do
      else
         do j = 1, npde
            uval(j)   = fbasis(kcol+nconti,  1) * y(j,ncpts)
            uxval(j)  = fbasis(kcol+nconti,  2) * y(j,ncpts)
     &                + fbasis(kcol+nconti-1,2) * y(j,ncpts-1)
            uxxval(j) = fbasis(kcol+nconti,  3) * y(j,ncpts)
     &                + fbasis(kcol+nconti-1,3) * y(j,ncpts-1)
     &                + fbasis(kcol+nconti-2,3) * y(j,ncpts-2)
         end do
      end if
      return
      end
...
c       by DASKR for a hot start, after calculating the bspline
...
c-----------------------------------------------------------------------
      ...
      subroutine jacd(t, y, yprime, pd, cj, rpar, ipar,
     &                derivf, bndxa, difbxa, bndxb, difbxb)
c-----------------------------------------------------------------------
c Purpose:
c       This is the subroutine which defines the Jacobian of the
c       differential/algebraic system to be solved by DASKR. It returns:
c                       PD := dG/dY + cj * dG/dY'
...
c                               rpar(ipar(ibasi)) stores the basis
c                               function values at the collocation
c                               points when using daskr_kcol.
c                               rpar(ipar(ibasi)) contains
c                               a three dimensional array A of size
...
c     Calculate Jacobian for daskr_kcol.
...
c-----------------------------------------------------------------------
c Purpose:
c       This subroutine is called by jacd. It provides a lower-level
...
c
        double precision        cj
c                               cj is a scalar chosen by DASKR to
...
c           Generate the approximate solution and its spatial
c           derivatives at the current collocation point.
            call eval(npde,kcol,ij,2,ncpts,work(iu),work(iux),
...
c       system to be solved by DASKR. It returns the residual (delta) of
c       the DAE system:
c                          delta := G(t, Y, Y')
c       The DAE G(t, Y, Y') = 0 arises from applying the method-of-lines
c       and bspline collocation to the system of NPDE PDES of
...
c
        double precision        cj
c                               cj is proportional to 1/h and is used
c                               to scale the algebraic variables to
c                               improve conditioning of the index-1
c                               DAE system.
c
        double precision        rpar(*)
c                               rpar is the BACOLI floating point work
...
c       Output:
        integer                 ires
c                               ires is a user flag set to alert DASKR
...
c     Calculate residual for daskr.
     ...
     &                  fbasis, t, y, yprime, work, delta, cj,
     &                  f, bndxa, bndxb)
...
c-----------------------------------------------------------------------
c     Scale the algebraic variables (associated with the boundary
c     conditions) by cj (proportional to 1/h) in order to improve the
c     conditioning of this index-1 DAE system.
...
c-----------------------------------------------------------------------
      ...
      subroutine lowint(kcol, nint, npde, istart, icount, xbs, x, y,
     &                  h, hd, g, bassc, basm, u, um, usol, lwork, lbw)
c-----------------------------------------------------------------------
...
c     The breakpoint sequence. This is required by DBSPVD to compute the
...
c
      double precision        y(npde*(nint*kcol+nconti))
c     B-spline coefficients computed by DASKR.
c
      integer                 lbw
c     lenbwk is the size of the work array for dbspvd and must satisfy:
c         lbw >= (kcol+nconti+1)*(kcol+nconti+2)/2
c
c-----------------------------------------------------------------------
c Inoutput
...
c        Call DBSPVD to obtain values of B-spline basis functions at the
c        internal interpolated points.
c        The subroutine DINTRV does not need to be called to find ileft,
...
c        There are no such points for the LOI scheme when kcol = 3.
         if (kcol .ne. 3) then
            ...
            do i = 1, nint
               ...
               do j = 1, kcol-3
                  jj = (i-1) * (kcol-3) + j
                  call dbspvd(xbs, kcol+nconti, 1, scpt(j), ileft,
     &                        kcol+nconti, bassc(1, jj), lwork)
               end do
            end do
         endif

c        Call DBSPVD for the values and first derivatives of the
...
c        so we need a separate call for the last mesh point.)
         ...
         do i = 1, nint
            ileft = ileft + kcol
            call dbspvd(xbs, kcol+nconti, 2, x(i), ileft,
     &                  kcol+nconti, basm(1, 1, i), lwork)
         end do
c        ileft = kcol+nconti+kcol*(nint-1)
         call dbspvd(xbs, kcol+nconti, 2, x(nint+1), ileft,
     &               kcol+nconti, basm(1, 1, nint+1), lwork)
...
c     of unnecessary calls to DBSPVD by saving some of the required
c     values.
      ii = 0
      do i = 1, nint
         do j = 1, kcol-3
...
c-----------------------------------------------------------------------
      ...
      subroutine scint(kcol, nint, npde, istart, icount, xbs, x, y,
     &                 h, hd, g, bassc, basm, u, um, usol, lwork, lbw)
c-----------------------------------------------------------------------
c     This subroutine evaluates a Hermite-Birkhoff (H-B) interpolant of
...
c        Call DBSPVD to obtain values of the B-spline basis functions at
...
c        d1 and d2 are what is *actually* added or subtracted
      ...
      subroutine rtbac(neq, t, y, yp, rt, nrt, rval, rpar, ipar, x)
c ----------------------------------------------------------------------
c     This subroutine is called by DASKR when the event detection mode
c     is used. Allows for the user to make use of a root function with
c     a simplified user interface which hides some of the implementation
c     details.
c ----------------------------------------------------------------------
          implicit none
...
c Offsets
          integer                 inint
          parameter              (inint  =  3)
c                                 ipar(inint) is the current number
c                                 of spatial subintervals.
c ----------------------------------------------------------------------
c Inputs
          integer                 neq
c                                 Current number of Bspline coefficients
c                                 being computed by DASKR.
c
          double precision        t
c                                 Current point in time.
c
          double precision        y(neq)
c                                 Current values of the Bspline
c                                 coefficients being computed by DASKR.
c
          double precision        yp(neq)
c                                 Derivatives of the Bspline
c                                 coefficients.
c
          integer                 nrt
c                                 Number of roots being searched for.
c
          double precision        rval(nrt)
c                                 Contains the current values of the
c                                 root function.
c
          double precision        rpar(*)
c                                 Floating point work array.
c
          integer                 ipar(*)
c                                 Integer point work array.
c
          double precision        x(*)
c                                 The current spatial mesh.
c
          external                rt
c                                 User function which defines the root
c                                 function
c ----------------------------------------------------------------------
c Locals

          integer                 nint
c                                 Current number of spatial mesh
c                                 subintervals
c ----------------------------------------------------------------------

C         Get the number of subintervals
          nint = ipar(inint)

C         Make the call to the user callback to compute the roots
          call rt(t, x, nint, y, yp, neq, rval, nrt)
      return
      end

c     dummy subroutines for DASKR
      subroutine psol
      end
...
c     Compute the arc-length monitor function
     ...
     &        - (h1)/((h1+h2)*h2)*wm(uboff+(j-1)))**2
      ...
      wm(moff) = sqrt(1.0d0+wm(moff)/dble(npde))
...
c     Internal subintervals.
     ...
     &            + (h1)/(h2*(h1+h2))*wm(uboff+(j-1)))**2
          ...
          wm(moff+(i-1)) = sqrt(1.0d0+wm(moff+(i-1))/dble(npde))
      end do
...
c     RHS boundary
     ...
     &        + (h1+2.0d0*h2)/(h2*(h1+h2))*wm(uboff+(j-1)))**2
      ...
      wm(moff+nint) = sqrt(1.0d0+wm(moff+nint)/dble(npde))
