...
c (i) The DASKR package (www.netlib.org/ode/daskr.tgz)
c Many of these subroutines have been modified for BACOLI's purposes.
c These modifications were based upon modifications that Rong Wang,
c author of BACOL, made to DASSL. The modifications add the
c Almost-Block-Diagonal (ABD) linear algebra routines of LAMPAK and
c code to approximate an ABD Jacobian matrix using forward differences.
c Several parameter lists are also modified so that BACOLI user
c subroutines may be passed down to any call to RES or JACD.
c TODO reference and license info

c (ii) From the SLATEC (www.netlib.org/slatec/) numerical software
c collection: dbspvd, dbspvn, dintrv
c SLATEC is in the public domain. It's disclaimer may be found here:
c www.netlib.org/slatec/src/aaaaaa.f
c These subroutines were modified to remove calls to XERMSG.
c For more on the B-spline routines, refer to:
c de Boor, C., Package for Calculating with B-splines,
c SIAM j. numer. anal., vol. 14, no. 3, june 1977, pp. 441-472.

c (iii) From Patrick Keast: gauleg, lamdec, lamsol
c TODO more info

c (iv) From the EISPACK (www.netlib.org/eispack/) numerical software
c collection: imtql1,imtql2,pythag.
c These routines seem to be available under a modified BSD or MIT
c style license, but we have not been able to locate said license.

c (v) From the LINPACK (www.netlib.org/linpack/) numerical software
c collection: dgbfa,dgbsl,dgefa,dgesl.
...
C                       IWORK(2)=MU ****
c=======================================================================
c           no (ABD) - set INFO(6) = 2
c                       and provide shape parameters npde, nint, kcol in
c                       IWORK(38)=npde
c                       IWORK(39)=kcol
c                       IWORK(40)=nint ****
c=======================================================================
...
c=======================================================================
c                 IF INFO(6) = 2 (ABD matrix),
c                    add npde*npde*(nint*kcol*(kcol+2)+4) + neq
c=======================================================================
...
C
      ...
      DIMENSION RPAR(*),IPAR(*)
      CHARACTER MSG*80
      EXTERNAL  RES, JAC, PSOL, RT, DDASID, DDASIK, DNEDD, DNEDK, rtu
c=======================================================================
      dimension jroot(*)
      integer lnpde, lkcol, lnint, nconti, npde, kcol, nint
      parameter (nconti = 2)
        integer                 ir0
        parameter              (ir0    =  3)
...
c=======================================================================
c     Pointers into IWORK describing ABD Jacobian structure.
      parameter (lnpde=38, lkcol=39, lnint=40)
c=======================================================================
C
...
C
         NCPHI = MAX(MXORD + 1, 4)
c=======================================================================
c        When checking the size of rwork, size requirements depend upon
c        the assumed structure of the iteration matrix.
c        mtype controls what blocks of code are executed within the
c        linear algebra routines DMATD and DSLVD.
         if (info(6) .eq. 2) then
            npde = iwork(lnpde)
            kcol = iwork(lkcol)
            nint = iwork(lnint)
            lenpd = npde*npde*(nconti+nconti+kcol*(kcol+nconti)*nint)
            lenrw = 60 + 3*nrt + (ncphi+3)*neq + lenpd + neq
            if (info(5) .eq. 0) then
               iwork(lmtype) = 7
            else
               iwork(lmtype) = 6
...
c=======================================================================
     *      DNEDD, f, derivf, bndxa, difbxa, bndxb, difbxb)
c=======================================================================
      ELSE IF (INFO(12) .EQ. 1) THEN
         CALL DDSTP(TN,Y,YPRIME,NEQ,
     *      RES,JAC,PSOL,H,RWORK(LWT),RWORK(LVT),INFO(1),IDID,RPAR,IPAR,
     ...
     *      IWORK(LK), IWORK(LKOLD),IWORK(LNS),NONNEG,INFO(12),
c=======================================================================
     *      DNEDK, f, derivf, bndxa, difbxa, bndxb, difbxb)
c=======================================================================
      ENDIF
C
...
C       write(*,*) 'calling rt'
      CALL RT (NEQ, RWORK(LT0), Y, YP, rtu, NRT, R0, RPAR, IPAR, xin)
C       write(*,*) 'out rt'
...
C R has a zero at T0.  Look at R at T0+ = T0 + (small increment). ------
 ...
 240  CALL RT (NEQ, RWORK(LT0), Y, YP, rtu, NRT, R0, RPAR, IPAR, xin)
      IWORK(LNRTE) = IWORK(LNRTE) + 1
      DO 250 I = 1,NRT
...
C Set T1 to TN or TOUT, whichever comes first, and get R at T1. --------
 ...
 330  CALL DDATRP (TN, T1, Y, YP, NEQ, KOLD, PHI, PSI)
      CALL RT (NEQ, T1, Y, YP, rtu, NRT, R1, RPAR, IPAR, xin)
      IWORK(LNRTE) = IWORK(LNRTE) + 1
C Call DROOTS to search for root in interval from T0 to T1. ------------
 ...
 350  CONTINUE
      ...
      CALL RT (NEQ, X, Y, YP, rtu, NRT, RX, RPAR, IPAR, xin)
      IWORK(LNRTE) = IWORK(LNRTE) + 1
      GO TO 350
 360  RWORK(LT0) = X
...
c=======================================================================
     *   IERNLS, f, derivf, bndxa, difbxa, bndxb, difbxb)
c=======================================================================
C
...
c=======================================================================
     *  NTYPE,NLS, f, derivf, bndxa, difbxa, bndxb, difbxb)
c=======================================================================
C
...
c=======================================================================
     *   NONNEG,NTYPE,IERNLS, f, derivf, bndxa, difbxa, bndxb, difbxb)
c=======================================================================
C
...
30    CONTINUE
      RETURN
*c=======================================================================
*     Is Rong Wang's optimisation still an improvement?
*     Saving a multiplication may no longer be worth the price in
*     memory accesses.
*c=======================================================================
*      implicit none
*      double precision v, rwt, rpar, sum, vmax
*      integer          neq, ipar, i
*c=======================================================================
*      DIMENSION V(*),RWT(*)
*      DIMENSION RPAR(*),IPAR(*)
...
c=======================================================================
     *  ICNFLG,ICNSTR,IERNLS, f, derivf, bndxa, difbxa, bndxb, difbxb)
c=======================================================================
C
...
C
      ...
      IWM(LNRE) = IWM(LNRE) + 1
c=======================================================================
      CALL RES(X,Y,YPRIME,CJ,DELTA,IRES,RPAR,IPAR, f, bndxa, bndxb)
c=======================================================================
...
c=======================================================================
     *     ICNFLG,ICNSTR,IERNEW, f, bndxa, bndxb)
c=======================================================================
C
...
c=======================================================================
     *                   ICNSTR, RLX, RPAR, IPAR, f, bndxa, bndxb)
c=======================================================================
C
...
c=======================================================================
     *              FNRMP, WM, IWM, RPAR, IPAR, f, bndxa, bndxb)
c=======================================================================
      IWM(LNRE) = IWM(LNRE) + 1
      IF (IRES .NE. 0) THEN
        IRET = 2
...
C-----------------------------------------------------------------------
      IRES = 0
c=======================================================================
      CALL RES(T,Y,YPRIME,CJ,R,IRES,RPAR,IPAR, f, bndxa, bndxb)
c=======================================================================
      IF (IRES .LT. 0) RETURN
C-----------------------------------------------------------------------
...
c=======================================================================
     *          TOLNEW,MULDEL,MAXIT,IRES,IDUM,IERNEW, f, bndxa, bndxb)
c=======================================================================
C
...
c=======================================================================
     *   S,CONFAC,TOLNEW,MULDEL,MAXIT,IRES,IDUM,IERNEW, f, bndxa, bndxb)
c=======================================================================
C
...
C
c=======================================================================
c     The extensive modifications to this subroutine make implicit
c     declarations too risky a proposition.
      implicit none
      double precision x, y, yprime, delta, cj, h, ewt, e, wm, rpar
      double precision uround, squr, del, delinv, ysave, ypsave
      integer          neq, iwm, ipar, ier, mtype, nrow, lenpd, ires
      integer          isave, ipsave, msave, mband, meband, mba, meb1
      integer          i, j, k, l, n, i1, i2, ii
      integer          lml, lmu, lmtype, lnre, lnpd, llciwp, lipvt
c=======================================================================
...
c     Pointers to sections of the ABD matrix in the work space.
      npdtop = 1
      npdblk = npdtop + siztop
      npdbot = npdblk + nint * sizblk

      GO TO (100,200,300,400,500,600,700),MTYPE
c=======================================================================
C
C
...
200   IRES=0
      ...
      DO 210 I=1,NEQ
         ...
         IWM(LNRE)=IWM(LNRE)+1
c=======================================================================
         CALL RES(X,Y,YPRIME,CJ,E,IRES,RPAR,IPAR, f, bndxa, bndxb)
c=======================================================================
         IF (IRES .LT. 0) RETURN
         DELINV=1.0D0/DEL
         DO 220 L=1,NEQ
...
c=======================================================================
600   continue
c     Construct analytic ABD Jacobian matrix (MTYPE=6).
      call jacd(x, y, yprime, wm, cj, rpar, ipar,
     &          derivf, bndxa, difbxa, bndxb, difbxb)
      goto 710
...
c           The bottom block is included if nint is odd.
         ...
         do i = i1, i2
            do n = i, neq, step
               ...
               del = squr * max(abs(y(n)), abs(h*yprime(n)), 1d0/ewt(n))
               del = sign(del, h*yprime(n))
               del = (y(n) + del) - y(n)
               y(n) = y(n) + del
...
710   continue
      call lamdec(neq, wm(npdtop), npde, 2*npde, wm(npdblk), kcol*npde,
     &            (kcol+nconti)*npde, nint, wm(npdbot), npde,
     &            iwm(lipvt), ier)
      return
c=======================================================================
C
...
c=======================================================================
      ...
      integer          neq, iwm, meband, mtype
      integer          lml, lmu, lmtype, llciwp, lipvt
c=======================================================================
...
c=======================================================================
      ...
      integer    npdtop, npdblk, npdbot, lwork
c=======================================================================
C
      LIPVT = IWM(LLCIWP)
      MTYPE=IWM(LMTYPE)
c=======================================================================
      GO TO(100,100,300,400,400,600,600),MTYPE
c=======================================================================
C
C     Dense matrix.
C
...
600   continue
      ...
      npdtop = 1
      npdblk = npdtop + npde*npde*nconti
      npdbot = npdblk + nint * npde*npde*kcol*(kcol+nconti)
      lwork  = npdbot + npde*npde*nconti

      call dcopy(neq, delta, 1, wm(lwork),1 )
      call lamsol(wm(npdtop), npde, 2*npde, wm(npdblk), kcol*npde,
     &            (kcol+nconti)*npde, nint, wm(npdbot), npde,
     &            iwm(lipvt), wm(lwork), delta)
c=======================================================================
C
...
c=======================================================================
     *   WM(LWP), IWM(LIWP), PWK, RPAR, IPAR, f, bndxa, bndxb)
c=======================================================================
      IWM(LNPS) = IWM(LNPS) + 1
      IF (IER .NE. 0) THEN
        IERNEW = 3
...
c=======================================================================
     *   PWK, ICNFLG, ICNSTR, RLX, RPAR, IPAR, f, bndxa, bndxb)
c=======================================================================
C
...
c=======================================================================
     *   WP, IWP, PWK, RPAR, IPAR, f, bndxa, bndxb)
c=======================================================================
      IWM(LNRE) = IWM(LNRE) + 1
...
c=======================================================================
     *   S,TEMP1,TOLNEW,MULDEL,MAXIT,IRES,IERSL,IERNEW, f, bndxa, bndxb)
c=======================================================================
C
      IF (IERNEW .GT. 0 .AND. JCALC .NE. 0) THEN
C
...
c=======================================================================
     1     IER, NRE, NPSL, RPAR, IPAR, f, bndxa, bndxb)
c=======================================================================
        IF (IRES .LT. 0) RETURN
        IF (IER .NE. 0) GO TO 300
...
C-----------------------------------------------------------------------
      CONTINUE
c=======================================================================
      CALL RES(TN,Z,YPTEM,CJ,VTEM,IRES,RPAR,IPAR, f, bndxa, bndxb)
c=======================================================================
      NRE = NRE + 1
      IF (IRES .LT. 0) RETURN
C-----------------------------------------------------------------------
...
c--- LAMPAK routines
c    (employed to factor and solve ABD Jacobians of DAE systems)
c    This replaces COLROW, which is subject to ACM copyright and
c    licensing.
...
c--- EISPACK routines
c    (called by GAULEG when find Gauss-Legendre points for the
...
c--- LINPACK routines
c    (dead code for dense and banded linear algebra in DDAJAC & DDASLV)
...
c--- BLAS routines
c    (may be removed in favor of linking an optimised implementation)
...
C
  200 CONTINUE
C     CALL XERMSG ('SLATEC', 'DBSPVD', 'K DOES NOT SATISFY K.GE.1', 2,
C    +   1)
      RETURN
  205 CONTINUE
C     CALL XERMSG ('SLATEC', 'DBSPVD',
C    +   'NDERIV DOES NOT SATISFY 1.LE.NDERIV.LE.K', 2, 1)
      RETURN
  210 CONTINUE
C     CALL XERMSG ('SLATEC', 'DBSPVD',
C    +   'LDVNIK DOES NOT SATISFY LDVNIK.GE.K', 2, 1)
      RETURN
      END
...
C
   90 CONTINUE
C     CALL XERMSG ('SLATEC', 'DBSPVN', 'K DOES NOT SATISFY K.GE.1', 2,
C    +   1)
      RETURN
  100 CONTINUE
C     CALL XERMSG ('SLATEC', 'DBSPVN',
C    +   'JHIGH DOES NOT SATISFY 1.LE.JHIGH.LE.K', 2, 1)
      RETURN
  105 CONTINUE
C     CALL XERMSG ('SLATEC', 'DBSPVN', 'INDEX IS NOT 1 OR 2', 2, 1)
      RETURN
  110 CONTINUE
C     CALL XERMSG ('SLATEC', 'DBSPVN',
C    +   'X DOES NOT SATISFY T(ILEFT).LE.X.LE.T(ILEFT+1)', 2, 1)
      RETURN
      END
