...
// lsoda
nfev = 0;
y_lsoda = ode(y0, t0, tspan, model_with_if);
E_lsoda = y_lsoda(2, :);
count_lsoda = nfev

// stiff
nfev = 0
y_stiff = ode("stiff", y0, t0, tspan, model_with_if);
E_stiff = y_stiff(2, :);
count_stiff = nfev

// rkf
nfev = 0
y_rkf = ode("rkf", y0, t0, tspan, model_with_if);
E_rkf = y_rkf(2, :);
...
// rk
nfev = 0
y_rk = ode("rk", y0, t0, tspan, model_with_if);
E_rk = y_rk(2, :);
count_rk = nfev

// adams
nfev = 0
y_adams = ode("adams", y0, t0, tspan, model_with_if);
E_adams = y_adams(2, :);
count_adams = nfev
