        ...
        while t_initial < 180:
            ...
            if (measures_implemented):
                ...
                sol = solve_ivp(model_with_measures, tspan, y_initial, events=root_10000, 
                        dense_output=True, atol=atol, rtol=rtol, method="RK45")
                end = timer()
                measures_implemented = False
            else:
                start = timer()
...
### plotting the data
# need to plot 1e-1 separately
tolerances_plot_without_event = [1e-2, 1e-4, 1e-6, 1e-7, 1e-8, 1e-10, 1e-11];
plt.figure(1)
for tolerance in tolerances_plot_without_event:
    print(tolerance)
    name = "no_event_" + str(abs(log10(tolerance)))
...
### plotting the code without event detection
plt.figure(3)
for tolerance in tolerances:
    print(tolerance)
    name = "with_event_" + str(abs(log10(tolerance)))
